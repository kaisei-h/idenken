...
for prm1 in [0]:
    for prm2 in [0]:
        net = model.new_Variable_3(num_layer=8, num_filters=128, kernel_sizes=7).to(device)
        ...
        torch.save(net.state_dict(), 'big_data3.pth')

        print(f'num_layer=8, num_filters=128, kernel_sizes=7')
        print('dilationで減らすパターン')
        ...
        plt.savefig(f'variable3_{val_loss_list[-1]:.2f}_{prm1}_{prm2}.png')
