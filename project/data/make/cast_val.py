...
for i in range(cnt):
    ...
    if (i==cnt/2):
        datePrint("やっと半分だよ")
print(f"saving to input_val{sys.argv[1]}.pkl")
pickle.dump(torch.stack(input_array), open(f"input_val{sys.argv[1]}.pkl", "wb"))
        
print(f"saving to target_val{sys.argv[1]}.pkl")
pickle.dump(torch.stack(target_array), open(f"target_val{sys.argv[1]}.pkl", "wb"))
