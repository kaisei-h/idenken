...
def model_device(device, model):
    print("device: ", device)
    model.to(device)
    model = torch.nn.DataParallel(model, device_ids=[3]) # make parallel
...
datatypes = ['repeat', 'long', 'stem', 'random']
modeltypes = [CNN, DCNN, TransCNN, RBERT_CNN]

for modeltype in modeltypes:

    max_length ,batch_size = 440, 1000
    ...
    for datatype in datatypes:
        ...
        for i in range(990,1000):
            val_seq_path.append(f'{seq_path}seq{i+1}.fa')
            val_acc_path.append(f'{acc_path}acc{i+1}.csv')      

    criterion = nn.MSELoss().to(device)

    config = get_config(file_path = "../RNABERT/RNA_bert_config.json")
    if modeltype==RBERT_CNN:
    ...
    result.plot_result(pad_removed_target, pad_removed_output, mode='save', name=f'mix_{modeltype}')

    dt_now = datetime.now(JST)
    dt_now = dt_now.strftime('%Y%m%d-%H%M%S')
    print(dt_now)
    with open(f'log/{dt_now}_mix_{modeltype}.log', 'w') as f:
