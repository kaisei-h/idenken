...
datatypes = ['random', 'random_stem', 'repeat_random', 'stem_repeat_random']

for datatype in reversed(datatypes):
    print(f'{datatype}')
    dt_list = datatype.split('_')
    ...
    for n in range(99):
        idx = n*10+1
        train_seq_paths.append([f'../../data/{dt_list[(idx+i)%len(dt_list)]}/sequence/seq{idx+i}.fa' for i in range(10)])
        train_acc_paths.append([f'../../data/{dt_list[(idx+i)%len(dt_list)]}/accessibility/acc{idx+i}.csv' for i in range(10)])

    val_seq_path = [f'../../data/{dt_list[(i+1)%len(dt_list)]}/sequence/seq{i+1}.fa' for i in range(990,1000)]
    val_acc_path = [f'../../data/{dt_list[(i+1)%len(dt_list)]}/accessibility/acc{i+1}.csv' for i in range(990,1000)]
    ...
    result.plot_result(pad_removed_target, pad_removed_output, mode='save', name=f'{datatype}')

    dt_now = datetime.now(JST)
    dt_now = dt_now.strftime('%Y%m%d-%H%M%S')
    print(dt_now)
    with open(f'log/{datatype}_{dt_now}.log', 'w') as f:
